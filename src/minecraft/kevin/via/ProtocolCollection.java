...
public enum ProtocolCollection {
    ...
    // $FF: synthetic field
    private static final ProtocolCollection[] pc = ProtocolCollection.values();
    @NotNull
    private final ProtocolVersion protocolVersion;

    ProtocolCollection(ProtocolVersion protocolVersion) {
        this.protocolVersion = protocolVersion;
    }

    @NotNull
    public static EnumEntries getEntries() {
        return EnumEntriesKt.enumEntries(ProtocolCollection::values);
    }

    public static ProtocolCollection getProtocolById(int id) throws NotFoundException {
        for (ProtocolCollection item : values()) {
            if (item.getProtocolVersion().getVersion() == id) {
                return item;
            }
        }
        throw new NotFoundException("getProtocolById cannot found exist protocol with id" + id); // Or handle no match found
    }
