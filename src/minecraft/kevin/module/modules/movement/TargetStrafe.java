...
/**
 * TargetStrafe module allows the player to strafe around a target entity while maintaining a specified radius.
 * It includes options for rendering, safe walking, and controlling the strafing behavior based on player input.
 */
public final class TargetStrafe extends ClientModule {
    ...
    public TargetStrafe() {
        super("TargetStrafe", "Strafe around your target.", ModuleCategory.MOVEMENT);
    }
    ...
    public void onMove(MoveEvent event) {
        if (safewalk.get() && mc.thePlayer.onGround && canStrafe(KevinClient.combatManager.getTarget())) {
    ...
    private boolean canStrafe(@Nullable EntityLivingBase target) {
        return this.getState() && target != null && (!space.get() || mc.thePlayer.movementInput.jump) && (!onlySpeed.get() || KevinClient.moduleManager.getModule(Speed.class).getState()) && (hurtTime.get() <= mc.thePlayer.hurtTime);
    }
}