...
object AutoDisable : ClientModule(
    "AutoDisable",
    "Auto disable modules.(Use Command .AutoDisableSet <ModuleName> <add/remove> <World/SetBack/All>)",
    ModuleCategory.MISC
),
    IClientCommand {
    ...
    private fun addUsageMessage(args: Array<out String>) =
        ChatUtils.messageWithStart("§cUsage: §9.autodisable §a${args[0]} ${args[1]} §c<World/SetBack/All>")

    private fun usageMessage() =
        ChatUtils.messageWithStart("§cUsage: .autodisable <ModuleName> <add/remove> <World/SetBack/All>")
    ...
    fun onWorld(event: WorldEvent) {
        KevinClient.moduleManager.getModules()
            ...
            .forEach {
                it.state = false
                if (notification.get()) KevinClient.hud.addNotification(
                    Notification(
                        "Auto disabled module ${it.name}.",
                        "AutoDisable",
